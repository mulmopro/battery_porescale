...
## Comsol Sodium-ion Battery Cell
...
- geometry_anode.txt & geometry_cathode.txt : these files contain the geometrical and spatial characterization of each particle. Each file is formed by four columns separated by spaces (' '). The first column is a capital letter followed by a closed round bracket and the letter can have three different values: D)=Dimension; P)=Position; R)=Rotation. The others three columns contains float numbers and each of them represent a dimension, a position or a rotation along an axes, in order, X, Y and, Z. The number of lines that start with "D)" must be equal either to the number of lines that start with "P)" or "R)" and this will correspond to the total number of particles genereted in the system.
- Eeq_Anode.txt & Eeq_Cathode.txt : these are a double column files of float numbers. The first column represent the State of Charge, so it must be 0<SoC<1. The second column is the experimental equilibrium potential of the electrod under consideration as function of the SoC.
- Ds_n.txt & Ds_p.txt : these files have a formatting similar to the Eeq_Anode.txt/Eeq_Cathode.txt, but they contain the solid diffusion coefficient as a function of the concentration of intercalated sodium in [kmol/m^3] in the negative electrode (Ds_n) and in the positive one (Ds_p).
- k_n.txt & k_p.txt: these files have the same formatting of Ds_n.txt/Ds_p.txt, but they contain the kinetic constant values of the intercalation reaction on the Anode/Electrolyte interface (k_n) and Cathode/Electrolyte interface (k_p) as function of the intercalated sodium concentration in the negative/positive electrode.
- D_L.txt : this is a double column file of float numbers. The first column represent the Lithium concentration within the electrolyte, the second column is the liquid diffusion coefficient as function of the concentration in [kmol/m^3].
- sigma_L.txt : this file has a formatting similar to the D_L.txt, but it contains the liquid electrical conductivity as a function of the concentration in [kmol/m^3].
...
```bash
[...]\comsol64\multiphysics\bin\comsol compile *.java
```
